...
namespace ProfileCAM.Core.GCodeGen.LCMMultipass2HLegacy {
   public class GCodeGenerator4LCMMultipass2HLegacy : IGCodeGenerator {
      ...
      public List<List<GCodeSeg>[]> FrameTraces {
         get => throw new Exception ("FrameTraces not implemented");
      }
      ...
      public EHeads Heads { get { return GCodeGenSettings.Heads; } set { GCodeGenSettings.Heads = value; } }
      ...
      public PartConfigType PartConfigType { get { return GCodeGenSettings.PartConfig; } set { GCodeGenSettings.PartConfig = value; } }
      ...
      void WriteBlockType (Tooling toolingItem, ToolingSegment? ts, ToolingSegment? nextTs,
         ...
         stNormalFlange = GetFlangeType (ts.Value.Vec0, GetXForm ());
         endNormalFlange = GetFlangeType (ts.Value.Vec1, GetXForm ());
         ...
         if (nextTs != null) {
            var isFlexCutSeg = IsFlexCutSegment (nextTs.Value);
            if (isFlexCutSeg) {
               nextSegEndNormalFlange = GetFlangeType (nextTs.Value.Vec1, GetXForm ());
         ...
         else             if (!isValidNotch && !isCutout || isFlangeCutout) {
               if (GetFlangeType (toolingItem, GetXForm ()) == EFlange.Web)                   if (IsOppositeReference (toolingItem.Name)) {
                     blockType = -3;
...
else if (GetFlangeType (toolingItem, GetXForm ()) == EFlange.Bottom) {
                  blockType = 1;
                  comment += " Bottom Flange Cut";
               } else if (GetFlangeType (toolingItem, GetXForm ()) == EFlange.Top) {
                  blockType = 2;
                  comment += " Top Flange Cut";
               } else if (toolingItem.IsFlexHole () || !isValidNotch)                   blockType = 0;
               ...
               switch (cutKind) {
                  ...
                  case ECutKind.Top2YNeg:
                     ...
                     if (stNormalFlange == EFlange.Top && endNormal == EFlange.Top)
                        ...
                        blockType = 4.9;
                     else if (stNormalFlange == EFlange.Web && endNormal == EFlange.Web)                         if (cutKind == ECutKind.Top2YPos)
                           blockType = -4.0;
                        else
                           blockType = 4.0;
                        ...
                        blockType = 4.5;
                     else if (stNormalFlange == EFlange.TopFlex && endNormal == EFlange.TopFlex)                         if (machiningSense == MachiningSense.Downward)
                           blockType = -4.5;
                        else
                           blockType = -4.6;
                        ...
                        blockType = 4.6;
                     else if (stNormalFlange == EFlange.BottomFlex && endNormal == EFlange.BottomFlex)                         if (machiningSense == MachiningSense.Downward)
                           blockType = 4.5;
                        else
                           blockType = 4.6;
else
                  ...
                  case ECutKind.YNegToYPos:
                     ...
                     /* TRIPLE_FLANGE_NOTCH */
                     stNormalFlange = GetArcPlaneFlangeType (toolingItem.Segs[0].Vec0, GetXForm ());
                     endNormalFlange = GetArcPlaneFlangeType (toolingItem.Segs[^1].Vec0, GetXForm ());
      ...
      public void ResetBookKeepers () {
         mPgmNo[EFlange.Web] = 3000;
         mPgmNo[EFlange.Top] = 2000;
         mPgmNo[EFlange.Bottom] = 1000;
      ...
      readonly Dictionary<EFlange, int> mPgmNo = new () {
         [EFlange.Web] = 3000,
         [EFlange.Top] = 2000,
         [EFlange.Bottom] = 1000
      };
      ...
      int GetProgNo (Tooling item) {
         ...
         else return mPgmNo[GetFlangeType (item, GetXForm ())];
      }
      ...
      public GCodeGenerator4LCMMultipass2HLegacy (IGenesysHub gHub, bool isLeftToRight) {
         mGHub = gHub;
         SetFromMCSettings ();
         It.OnSettingValuesChangedEvent += SetFromMCSettings;
         Traces = [[], []];
      ...
      GCodeGenerator4LCMMultipass2HLegacy () { }
      ...
      public static void EvaluateToolConfigXForms (Workpiece work) {
         // For LH Component
         if (sXformLHInv == null || sXformRHInv == null) {
            sXformLHInv = new XForm4 ();
            sXformRHInv = new XForm4 ();
            var flangeThickness = work.Model.Entities.OfType<E3Plane> ().ToList ().First ().ThickVector.Length;
            sXformLHInv.Translate (new Vector3 (0.0, work.Bound.YMin, flangeThickness));
            //if (mcName == "LMMultipass2H")
            //Utils.sXformLHInv.SetRotationComponents (new Vector3 (-1, 0, 0), new Vector3 (0, -1, 0), new Vector3 (0, 0, 1));
            sXformLHInv.Invert ();
            // For RH component
            sXformRHInv.Translate (new Vector3 (0.0, work.Bound.YMax, flangeThickness));
            //if (mcName == "LMMultipass2H")
      ...
      public static Point3 XfmToMachine (IGCodeGenerator codeGen, Point3 ptWRTWCS) {
         Vector3 resVec;
         if (codeGen.PartConfigType == PartConfigType.LHComponent) resVec = sXformLHInv * ptWRTWCS;
         else resVec = sXformRHInv * ptWRTWCS;
         return Geom.V2P (resVec);
      }

      public Point3 XfmToMachine (Point3 ptWRTWCS) {
         Vector3 resVec;
         if (PartConfigType == PartConfigType.LHComponent) resVec = sXformLHInv * ptWRTWCS;
         else resVec = sXformRHInv * ptWRTWCS;
         return Geom.V2P (resVec);
      }
      ...
      public static XForm4 XfmToMachine (IGCodeGenerator codeGen, XForm4 xFormWCS) {
         XForm4 mcXForm;
         if (codeGen.PartConfigType == PartConfigType.LHComponent) mcXForm = sXformLHInv * xFormWCS;
         else mcXForm = sXformRHInv * xFormWCS;
         return mcXForm;
      }

      public XForm4 XfmToMachine (XForm4 xFormWCS) {
         XForm4 mcXForm;
         if (PartConfigType == PartConfigType.LHComponent) mcXForm = sXformLHInv * xFormWCS;
         else mcXForm = sXformRHInv * xFormWCS;
         return mcXForm;
      }
      ...
      public static Vector3 XfmToMachineVec (IGCodeGenerator codeGen, Vector3 vecWRTWCS) {
         Vector3 resVec;
         if (codeGen.PartConfigType == PartConfigType.LHComponent) resVec = sXformLHInv * vecWRTWCS;
         else resVec = sXformRHInv * vecWRTWCS;
         return resVec;
      }

      public Vector3 XfmToMachineVec (Vector3 vecWRTWCS) {
         Vector3 resVec;
         if (PartConfigType == PartConfigType.LHComponent) resVec = sXformLHInv * vecWRTWCS;
         else resVec = sXformRHInv * vecWRTWCS;
         return resVec;
      }
      #endregion
      ...
      public double GetXPartition (List<Tooling> cuts) {
         ...
         double midX = -10;
         if (resHead0.Count > 0 && resHead1.Count > 0)             if (LeftToRightMachining) midX = (resHead0[^1].XMax + resHead1.First ().XMin) / 2.0;
            else midX = (resHead0[^1].XMin + resHead1.First ().XMax) / 2.0;
else if (resHead0.Count > 0)             if (LeftToRightMachining) midX = resHead0[^1].XMax;
            else midX = resHead0[^1].XMin;
else if (resHead1.Count > 0)             if (LeftToRightMachining) midX = resHead1[^1].XMax;
      ...
      public void CreatePartition (List<Tooling> cuts, bool optimize, Bound3 bound) {
         ...
         if (Machine == MachineType.LCMMultipass2H && bound.XMax - mXSplit < MinThresholdForPartition)             for (int ii = 0; ii < cuts.Count; ii++) {
               var cut = cuts[ii];
               cut.Head = 0;
               cuts[ii] = cut;
            }
      ...
      void Partition (List<Tooling> cuts, double ratio, Bound3 bound) {
         ...
         foreach (Tooling tooling in cuts)             if (IsSingleHead)                if (IsSingleHead1)                   if (Heads is EHeads.Left or EHeads.Both)
                     tooling.Head = 0;
                  else if (Heads is EHeads.Right or EHeads.Both)
                     tooling.Head = 1;
                  else throw new InvalidOperationException
                        ("Only single head detected while the tool configuration with Both tools prescribed!");
else                if (tooling.End.Pt.X < xf)
                  tooling.Head = 0;
               else
                  tooling.Head = 1;
      }
      ...
      public void WriteNSequenceHeader (EFlange p, int np, int cnt) {
         int startValue;
         int endValue;
         if (!OptimizeSequence)
      ...
      public int GenerateGCode (IGCodeGenerator.ToolHeadType head, List<List<Tooling>> frames, double xPartition) { // List<Tooling> is One Frame
         ...
         using (sw = new StreamWriter (head == 0 ? DINFileNameHead1 : DINFileNameHead2)) {
            ...
            sw.WriteLine ($"Job_Type  = {(PartConfigType == PartConfigType.LHComponent ? 1 : 2)}");
            ...
            sw.WriteLine (GetGCodeComment ($"Version = {It.Version}"));
      ...
      public void SetProgNo (Tooling toolingItem, int number) {
         if (toolingItem.IsHole ()) mPgmNo[GetFlangeType (toolingItem, GetXForm ())] = number;
      ...
      public void SetProgNo (Tooling toolingItem) {
         if (toolingItem.IsHole ()) mPgmNo[GetFlangeType (toolingItem, GetXForm ())] = mProgramNumber;
      ...
      public XForm4 GetXForm () {
         if (sXformLHInv == null || sXformRHInv == null)
            EvaluateToolConfigXForms (Process.Workpiece);
         return PartConfigType == PartConfigType.LHComponent ? sXformLHInv : sXformRHInv;
      }
      ...
      public static XForm4 GetXForm (Workpiece wp, IGCodeGenerator gcGen = null) {
         ...
         if (sXformLHInv == null || sXformRHInv == null)
            EvaluateToolConfigXForms (wp);
         if (gcGen == null)
            return It.PartConfig == PartConfigType.LHComponent ? sXformLHInv : sXformRHInv;
         return gcGen.PartConfigType == PartConfigType.LHComponent ? sXformLHInv : sXformRHInv;
      }
      ...
      public void WriteCurve (FCCurve3 fcCurve, Vector3 stNormal, Vector3 endNormal,
         ...
         var currPlaneType = GetArcPlaneType (endNormal, XForm4.IdentityXfm);
      ...
      public void WriteArc (FCArc3 fcArc, EPlane arcPlaneType, EFlange arcFlangeType,
         ...
         var apn = arcPlaneType switch {
            EPlane.Top => XForm4.mZAxis,
            EPlane.YPos => XForm4.mYAxis,
            EPlane.YNeg => -XForm4.mYAxis,
         ...
         if (arcSense == EArcSense.CCW) arcType = EArcSense.CCW;
         else arcType = EArcSense.CW;
         arcStartPoint = MovePoint (arcStartPoint, startNormal, Standoff);
         arcEndPoint = MovePoint (arcEndPoint, startNormal, Standoff);
         arcCenter = MovePoint (arcCenter, startNormal, Standoff);
         ...
         var mcCoordArcCenter2D = ToPlane (mcCoordArcCenter, arcPlaneType);
         var mcCoordArcStPoint2D = ToPlane (mcCoordArcStPoint, arcPlaneType);
         var mcCoordArcEndPoint2D = ToPlane (mcCoordArcEndPoint, arcPlaneType);
         ...
         switch (arcFlangeType) {
            case EFlange.Web:
               if (IsArc (fcArc))
                  ArcMachining (sw, arcType, arcSt2CenVec.X, OrdinateAxis.Y, arcSt2CenVec.Y, mcCoordArcEndPoint2D.X,
                     mcCoordArcEndPoint2D.Y, arcFlangeType, PartConfigType,
                  PartConfigType == PartConfigType.LHComponent ? sXformLHInv.InvertNew () : sXformRHInv.InvertNew ());
               else
                  CircularMachining (sw, arcType, arcSt2CenVec.X, OrdinateAxis.Y, arcSt2CenVec.Y, arcFlangeType, PartConfigType,
                  PartConfigType == PartConfigType.LHComponent ? sXformLHInv.InvertNew () : sXformRHInv.InvertNew (),
                     machine: Machine,
            ...
            case EFlange.Bottom:
               if (IsArc (fcArc))
                  ArcMachining (sw, arcType, arcSt2CenVec.X, OrdinateAxis.Z, arcSt2CenVec.Y, mcCoordArcEndPoint2D.X,
                     mcCoordArcEndPoint2D.Y, arcFlangeType, PartConfigType,
                  PartConfigType == PartConfigType.LHComponent ? sXformLHInv.InvertNew () : sXformRHInv.InvertNew ());
               else
                  CircularMachining (sw, arcType, arcSt2CenVec.X, OrdinateAxis.Z, arcSt2CenVec.Y, arcFlangeType, PartConfigType,
                  PartConfigType == PartConfigType.LHComponent ? sXformLHInv.InvertNew () : sXformRHInv.InvertNew (),
                     machine: Machine,
                     mcCoordArcCenter, radius, mcCoordArcStPoint);
               break;
      ...
      void WriteArc (FCArc3 fcArc, EPlane arcPlaneType,
         ...
         arcStartPoint = MovePoint (arcStartPoint, startNormal, GCodeGenSettings.Standoff);
         arcEndPoint = MovePoint (arcEndPoint, startNormal, GCodeGenSettings.Standoff);
         arcCenter = MovePoint (arcCenter, startNormal, GCodeGenSettings.Standoff);
         ...
         if (relativeCoords)             if (refStPt != null) {
               var refStPt2D = ToPlane (XfmToMachine (refStPt.Value), arcPlaneType);
...
#endif
            LinearMachining (sw, ptDiff.X, ptDiff.Y, ptDiff.Z, mcCoordTSAngleWithFlexRefStart,
...
#endif
            LinearMachining (sw, mcOuterFlexPt.X, mcOuterFlexPt.Y, mcOuterFlexPt.Z,
      ...
      public void WriteLineSeg (
       ...
       Point3? refStPoint /*= null*/) {
         ...
         if (relativeCoords)             if (refStPoint != null)
...
#endif
         // This following check does not set angle every time for the same plane type.
         if (currPlaneType == EPlane.Flex || currPlaneType != previousPlaneType) {
            if (currPlaneType == EPlane.Flex)
               angleBetweenZAxisAndCurrToolingEndPoint = GetAngleAboutXAxis (
                   XForm4.mZAxis, endNormal, GetXForm ()).R2D ();
            else
               angleBetweenZAxisAndCurrToolingEndPoint = GetAngle4PlaneTypeAboutXAxis (currPlaneType).R2D ();


            if (currFlangeType == EFlange.Bottom || currFlangeType == EFlange.Top)
               LinearMachining (
                   sw,
            ...
            else if (currFlangeType == EFlange.Web)
               LinearMachining (
                   sw,
                   mcCoordEndPointWithMCClearance.X,
                   OrdinateAxis.Y,
      ...
      public void WriteLineSeg (Point3 stPoint, Point3 endPoint, Vector3 startNormal, Vector3 endNormal,
         ...
         Vector3 stN, endN;
         if (PartConfigType == PartConfigType.LHComponent) {
            stN = sXformLHInv * startNormal;
            endN = sXformLHInv * endNormal;
         } else {
            stN = sXformRHInv * startNormal;
            endN = sXformRHInv * endNormal;
         }
         var cross = Geom.Cross (stN, endN).Normalized ();
         if (cross.Opposing (XForm4.mXAxis)) angleBetweenZAxisAndCurrToolingEndPoint *= -1;

         EFlange currFlangeType = GetArcPlaneFlangeType (startNormal, GetXForm ());
         if (planeChange)             if (currFlangeType == EFlange.Bottom || currFlangeType == EFlange.Top)
               LinearMachining (sw, mcCoordEndPointWithMCClearance.X, OrdinateAxis.Z, mcCoordEndPointWithMCClearance.Z,
                  angleBetweenZAxisAndCurrToolingEndPoint, lineSegmentComment, createDummyBlock4Master: CreateDummyBlock4Master);
            else if (currFlangeType == EFlange.Web)
               LinearMachining (sw, mcCoordEndPointWithMCClearance.X, OrdinateAxis.Y, mcCoordEndPointWithMCClearance.Y,
                  angleBetweenZAxisAndCurrToolingEndPoint, lineSegmentComment, createDummyBlock4Master: CreateDummyBlock4Master);
            else
               LinearMachining (sw, mcCoordEndPointWithMCClearance.X, mcCoordEndPointWithMCClearance.Y, mcCoordEndPointWithMCClearance.Z,
                  angleBetweenZAxisAndCurrToolingEndPoint, lineSegmentComment, createDummyBlock4Master: CreateDummyBlock4Master);
else             if (currFlangeType == EFlange.Top || currFlangeType == EFlange.Bottom)
               LinearMachining (sw, mcCoordEndPointWithMCClearance.X, OrdinateAxis.Z, mcCoordEndPointWithMCClearance.Z,
      ...
      public List<ToolingSegment> AddLeadinToTooling (Tooling toolingItem) {
         ...
         if (!toolingItem.IsNotch () && !toolingItem.IsMark ()) {
            // E3Plane normal 
            var apn = GetEPlaneNormal (toolingItem, XForm4.IdentityXfm);
            ...
            double circleRad;
            if (toolingItem.Segs.ToList ()[0].Curve is FCArc3 fcArc && IsCircle (fcArc)) {
            ...
            (firstToolingEntryPt, materialRemovalDirection) = GetMaterialRemovalSideDirection (toolingItem);
            ...
            if (IsCircle (toolingSegmentsList[0].Curve)) {
      ...
      public ToolingSegment? WriteTooling (List<ToolingSegment> toolingSegmentsList, Tooling toolingItem,
         ...
         EPlane previousPlaneType = EPlane.None;
         EPlane currPlaneType;
         if (toolingItem.IsFlexFeature ()) currPlaneType = EPlane.Flex;
         else currPlaneType = GetFeatureNormalPlaneType (CurveStartNormal, new ());
         ...
         EnableMachiningDirective ();
         {
            ...
            for (int ii = 0; ii < toolingSegmentsList.Count; ii++) {
               ...
               if (ii > 0) currPlaneType = GetFeatureNormalPlaneType (endNormal, GetXForm ());

               if (FCCurve is FCArc3) { // This is a 2d arc. 
                  var arcPlaneType = GetArcPlaneType (startNormal, GetXForm ());
                  var arcFlangeType = GetArcPlaneFlangeType (startNormal, GetXForm ());
                  ...
                  GetFlangeType (toolingItem, new ()), toolingItem.Name, lineSegmentComment:"", relativeCoords: relativeCoords,
                  refStPoint:null);
      ...
      static EToolingShape GetToolingShapeKind (Tooling toolingItem) {
         ...
         if (firstCurve as FCArc3 != null)             if (IsCircle (firstCurve))
      ...
      void MoveFromRetractToSafety (List<ToolingSegment> prevToolingSegs, string prevToolingName,
         ...
         if (prevToolingSegs != null && prevToolingSegs.Count > 0) {
            ...
            var prevToolingEPRetracted =
                   MovePoint (prevSegEndCurve.End, prevSegEndCurveEndNormal, mRetractClearance);
         ...
         var currToolingSPRetracted =
                MovePoint (currSegStCurve.Start, currSegStCurveStNormal, mRetractClearance);
      ...
      public void MoveFromSafetyToRetract (Point3 toolingStartPt, Vector3 toolingStartNormalVec, string toolingName,
         ...
         var currToolingStPtRetracted =
               MovePoint (toolingStartPt, toolingStartNormalVec, mRetractClearance);
         var angleBetweenZAxisNcurrToolingStPt =
       GetAngleAboutXAxis (XForm4.mZAxis, toolingStartNormalVec, GetXForm ()).R2D ();
         var mcCoordsCurrToolingStPtRetracted = XfmToMachine (currToolingStPtRetracted);

         if (featType != EKind.Mark) {
            if (planeChangeNeeded)
               LinearMachining (sw, mcCoordsCurrToolingStPtRetracted.X, mcCoordsCurrToolingStPtRetracted.Y,
      ...
      public void WriteBounds (Tooling toolingItem, List<ToolingSegment> segments, int startIndex = -1, int endIndex = -1) {
         var toolingSegsBounds = GetToolingSegmentsBounds (segments, Process.Workpiece.Model.Bound, startIndex, endIndex);
         ...
         var mcMinPt = XfmToMachine (this, minPt);
         var mcMaxPt = XfmToMachine (this, maxPt);
         double toolingLen;
         if (segments.Count == 2 && segments[1].Curve is FCArc3) {
            FCArc3 fcArc = segments[1].Curve as FCArc3;
            if (IsCircle (fcArc))
               toolingLen = GetToolingLength (segments, startIndex, endIndex, leadIn: true);
            else
               toolingLen = GetToolingLength (segments, startIndex, endIndex, leadIn: false);
         } else
            toolingLen = GetToolingLength (segments, startIndex, endIndex, leadIn: false);
      ...
      void CalibrateForCircle (Tooling toolingItem, Tooling prevToolingItem) {
         if (toolingItem.IsCircle ()) {
            ...
            var point2 = ToPlane (arcMcCoordsCenter, GetFeatureNormalPlaneType (toolingItem.Start.Vec, XForm4.IdentityXfm));
      ...
      public void WriteBounds (List<Point3> pts) {
         var toolingSegsBounds = GetPointsBounds (pts);
         var xMin = toolingSegsBounds.XMin; var xMax = toolingSegsBounds.XMax;
         double tl = 0;
      ...
      public void InitializeToolingBlock (Tooling toolingItem, Tooling prevToolingItem, /*double frameFeed,*/
         ...
         int offset = 0;
         switch (GetFlangeType (toolingItem, GetXForm ())) {
            case EFlange.Top:
               offset = 2;
         ...
         if (GetPlaneType (toolingItem, GetXForm ()) == EPlane.Top) sw.WriteLine ($"G93 Z0 T1");
         ...
         else if (GetFlangeType (toolingItem, GetXForm ()) == EFlange.Web)
      ...
      public void InitializeNotchToolingBlock (Tooling toolingItem, Tooling prevToolingItem,
         ...
         sw.WriteLine ();
         int offset;
         switch (GetArcPlaneFlangeType (segmentNormal, GetXForm ())) {
            case EFlange.Top:
               offset = 2;
         ...
         // Output X tool compensation
         if (GetArcPlaneType (segmentNormal, GetXForm ()) == EPlane.Top) sw.WriteLine ($"G93 Z0 T1");
         else sw.WriteLine ($"G93 Z=-Head_Height T1");
      ...
      public void PrepareforToolApproach (Tooling toolingItem, List<ToolingSegment> modifiedToolingSegs,
         ...
         if (isValidNotch) {
            ...
            if (prevToolingSegment != null)
               ...
               toolingItem.Name, firstTooling, toolingItem.Kind, usePingPongOption:true);
            else
...
#nullable restore
      ...
      public void WriteToolDiaCompensation (bool isFlexTooling) {
         ...
         if (!isFlexTooling)
            sw.WriteLine ($"G{(sXformRHInv[1, 3] < 0.0 ? 41 : 42)} D1 R=KERF E0\t( Tool Dia Compensation)");
      }
      ...
      public double GetTotalToolingsLength (List<Tooling> toolingItems) {
         ...
         foreach (var ti in toolingItems)             if (ti.EdgeNotch)
               continue;
            else
      ...
      void DoWriteCuts (
       List<Tooling> toolingItems, Bound3 bound, /*double frameFeed,*/ double xStart, double xPartition,
         ...
         mProgramNumber = mPgmNo[EFlange.Web];
         ...
         for (int i = 0; i < toolingItems.Count; i++) {
            ...
            if (toolingItem.IsHole () && !toTreatAsCutOut || toolingItem.IsMark ())                feature = new Hole (
            ...
            if (first) {
               string ncname = NCName;
               if (ncname.Length > 20) ncname = ncname[..20];
               if (!CreateDummyBlock4Master)                   if (shouldOutputDigit)
            ...
            if (!toolingItem.IsMark ())                if (toolingItem.IsNotch () && !toolingItem.EdgeNotch)
         ...
         if (shouldOutputDigit) {
            ...
            for (int i = range.Item1; i < range.Item2; i++) {
               ...
               string mark = PartConfigType == PartConfigType.LHComponent ? "L160" : "L161";
               sw.WriteLine ($"S100\r\nG93 X=P1838 Y=P1839\r\nG22 {mark} J=P1708\r\nG61\t( Stop Block Preparation )\r\n");
            }
         }
      }
      ...
      public void RapidPositionWithClearance (
       ...
       bool usePingPongOption = true, bool firstWJTTrace = true) {
         var toPointOffset = MovePoint (toPoint, endNormal, clearance);
         var angle = GetAngleAboutXAxis (XForm4.mZAxis, endNormal, GetXForm ()).R2D ();
         var mcCoordsToPointOffset = XfmToMachine (toPointOffset);

         if (!isMark) {
            EPlane currPlaneType = GetArcPlaneType (endNormal, GetXForm ());
      ...
      bool firstWJTTrace// true
         ...
         EPlane currPlaneType = GetArcPlaneType (wjtSeg.Vec1, GetXForm ());
         ...
         if (scrapSideNormal.Dot (XForm4.mNegZAxis).SGT (0))             if (nextMachiningStart.Z.SLT (Process.Workpiece.Bound.ZMin))
               nextMachiningStart = wjtSeg.Curve.End + scrapSideNormal.Normalized () * notchApproachDistance * 0.5;
else if (scrapSideNormal.Dot (XForm4.mXAxis).SGT (0))             if (nextMachiningStart.X.SGT (Process.Workpiece.Bound.XMax))
               nextMachiningStart = wjtSeg.Curve.End + scrapSideNormal.Normalized () * notchApproachDistance * 0.5;
else if (scrapSideNormal.Dot (XForm4.mNegXAxis).SGT (0))             if (nextMachiningStart.X.SLT (Process.Workpiece.Bound.XMin))
               nextMachiningStart = wjtSeg.Curve.End + scrapSideNormal.Normalized () * notchApproachDistance * 0.5;
         ...
         prevRapidPos = MovePoint (nextMachiningStart, wjtSeg.Vec0.Normalized (), mRetractClearance);
      ...
      public void MoveToNextTooling (
       ...
       bool usePingPongOption /*= true*/) {
         ...
         if (firstTime)
            changeInAngle = GetAngleAboutXAxis (
                XForm4.mZAxis,
         ...
         } else             if (featType != EKind.Mark) {
               sw.WriteLine ("ToolPlane\t( Confirm Cutting Plane )");

               if (CreateDummyBlock4Master)                   return;
            }

         if (!movedToCurrToolingRetractedPos)             MoveFromSafetyToRetract (
                nextToolingStartPoint,
                nextToolingStartNormal,
                nextToolingItemName,
                planeChangeNeeded,
      ...
      public void MoveFromRetractToSafety (ToolingSegment? prevToolingLastSegment, string prevToolingName,
         ...
         // Move to the current tooling item start posotion safeZ
         var currToolingSPRetracted =
                MovePoint (currToolingStPoint, currToolingStNormal, mRetractClearance);
         Point3 currToolingSPRetractedSafeZ = new (currToolingSPRetracted.X, currToolingSPRetracted.Y,
            mSafeClearance);
         var mcCoordsCurrToolingSPRetractedSafeZ = XfmToMachine (currToolingSPRetractedSafeZ);
         if (featType != EKind.Mark) {
      ...
      public bool IsOppositeReference (string toolingName) {
         if (WPOptions == null) {
            ...
            try {
               ...
               WPOptions = [];
               foreach (var item in data)                   WPOptions[item.FileName] = item;
            } catch (Exception) {
               return false;
            }
         }
      ...
      public void RapidMoveToPiercingPosition (
       Point3 stPoint, Vector3 stNormal, EKind featType, bool usePingPongOption, string comment) {
         // Debug
         if (RapidMoveToPiercingPositionWithPingPong && usePingPongOption)
            return;
         if (CreateDummyBlock4Master) return;
         var stPointWithStandoff =
               MovePoint (stPoint, stNormal, mRetractClearance);
         var mcCoordsStPoint = XfmToMachine (stPointWithStandoff);
         var planeType = GetPlaneType (stNormal, GetXForm ());
         comment = "Move to Piercing Position " + comment;
         if (planeType == EPlane.YNeg || planeType == EPlane.YPos)             RapidPosition (
                sw, mcCoordsStPoint.X, OrdinateAxis.Z, mcCoordsStPoint.Z,
                comment,
                UsePingPong && usePingPongOption ? "M1014" : "",
                createDummyBlock4Master: CreateDummyBlock4Master);
else if (planeType == EPlane.Top)             if (featType != EKind.Mark)
               RapidPosition (
