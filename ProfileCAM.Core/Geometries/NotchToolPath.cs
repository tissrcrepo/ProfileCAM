...
namespace ProfileCAM.Core.Geometries {
   ...
   public class NotchToolPath {
      ...
      public static (int segIndex, double param, Vector3 normal, double segParam)? ParamAtPosition (List<ToolingSegment> toolingSegs,
         Point3 pt, double tolerance = 1e-6) {
         double accumulatedLength = 0;

         for (int ii = 0; ii < toolingSegs.Count; ii++) {
            var segment = toolingSegs[ii];
            var curve = segment.Curve;
            var normal = toolingSegs[ii].Vec0.Normalized ();
            if (Geom.IsPointOnCurve (curve, pt, normal, hintSense: EArcSense.Infer, tolerance, true)) {
               // Check if point is at segment start (within tolerance)
               bool atStart = pt.DistTo (curve.Start).EQ (0);
               bool atEnd = pt.DistTo (curve.End).EQ (0);
               ...
               var pathLength = toolingSegs.Sum(seg => seg.Length);
               // Case 2: Any other segment's start point → return previous segment
               if (ii > 0 && atStart) {
                  double prevSegmentLength = toolingSegs[ii - 1].Length;
                  double np = (accumulatedLength - prevSegmentLength) / pathLength;
                  return (ii - 1, np, toolingSegs[ii - 1].Vec1.Normalized (), 1.0);
               }

               // Case 3: Any segment's end point
               if (atEnd) {
                  double np = (accumulatedLength + segment.Length) / pathLength;
                  return (ii, np, toolingSegs[ii].Vec1.Normalized (), 1.0);
               }
               ...
               double normalizedParam = (accumulatedLength + segLengthUptoPt) / pathLength;
               var segParam = segLengthUptoPt / curve.Length;
               normal = toolingSegs[ii].Vec0 * (1 - segParam) + toolingSegs[ii].Vec1 * segParam;
               return (ii, normalizedParam, normal, segParam);
            }
      ...
      public static (int segIndex, Point3 position, Vector3 normalAtPosition, double segParam)? PositionAtParam (List<ToolingSegment> toolingSegs, double param, double tolerance = 1e-6) {
         if (toolingSegs.Count == 0)
            throw new InvalidOperationException ("ToolPath contains no segments");
         var pathLength = toolingSegs.Sum(seg=>seg.Length);
         // Handle special case: param = 0
         if (param.EQ (0, tolerance))
            return (0, toolingSegs[0].Curve.Start, toolingSegs[0].Vec0.Normalized (), 0.0);

         // Handle case where param ≈ 1.0 (end of last segment)
         if (param.EQ (1.0, tolerance)) {
            int lastIdx = toolingSegs.Count - 1;
            return (lastIdx, toolingSegs[lastIdx].Curve.End, toolingSegs[0].Vec1.Normalized (), 1.0);
         }
         ...
         for (int ii = 0; ii < toolingSegs.Count; ii++) {
            ...
            if (targetLength.LTEQ (accumulatedLength + segmentLength, tolerance)) {
               ...
               // Case 1: At start of segment (only possible for ii > 0)
               if (ii > 0 && remainingLength.EQ (0, tolerance))
                  return (ii - 1, toolingSegs[ii].Curve.Start, toolingSegs[ii].Vec0.Normalized (), 0.0);


               // Case 2: At end of segment
               if (remainingLength.EQ (segmentLength, tolerance))
                  return (ii, toolingSegs[ii].Curve.End, toolingSegs[ii].Vec1.Normalized (), 1.0);
               ...
               // Interpolate normal
               var t = remainingLength / segment.Length;
               var normal = toolingSegs[ii].Vec0.Normalized () * (1 - t) + toolingSegs[ii].Vec1.Normalized () * t;
               return (ii, position, normal, t);
            }
      ...
      public static (int segIndex, Point3 position, Vector3 normalAtPosition, double segParam)? PositionFromPointAtLength (
         List<ToolingSegment> toolingSegs,
          Point3 fromPt,
          double lengthFromFromPt,
          double tolerance = 1e-6) {
         var pathLength = toolingSegs.Sum(seg=>seg.Length);
         // Check if the from point is on any of the tooling segment
         var fromPtOnAnySeg = toolingSegs.Any (x => Geom.IsPointOnCurve (x.Curve, fromPt, x.Vec0.Normalized (), hintSense: EArcSense.Infer, tolerance));
         if (!fromPtOnAnySeg)
            throw new ArgumentException ("Starting point not found on tool path", nameof (fromPt));

         // 1. First check if fromPt matches any segment's end point and lengthFromFromPt is 0
         var endSegmentIndex = toolingSegs.FindIndex (ts => ts.Curve.End.DistTo (fromPt).EQ (0, tolerance));
         if (endSegmentIndex != -1 && lengthFromFromPt.EQ (0, tolerance)) {
            if (toolingSegs[endSegmentIndex].Curve is FCArc3) {
               int aa = 0;
               ++aa;
            }
            // Return the end point of the matching segment
            return (endSegmentIndex, toolingSegs[endSegmentIndex].Curve.End, toolingSegs[endSegmentIndex].Vec1.Normalized (), 1.0);
         }

         // 2. Find the starting position on the path
         var startPos = NotchToolPath.ParamAtPosition (toolingSegs, fromPt, tolerance);
         if (!startPos.HasValue)
            throw new ArgumentException ("Starting point not found on tool path", nameof (fromPt));
         ...
         // 3. Calculate the exact starting length along the path
         for (int ii = 0; ii < startSegIndex; ii++)
            accumulatedLength += toolingSegs[ii].Length;

         var startSegment = toolingSegs[startSegIndex];
         if (toolingSegs[startSegIndex].Curve is FCArc3) {
            int aa = 0;
            ++aa;
         }
         double lengthInStartSegment = startSegParam * startSegment.Length;
         ...
         if (targetLength.EQ (pathLength, tolerance)) {
            int lastIdx = toolingSegs.Count - 1;
            if (toolingSegs[lastIdx].Curve is FCArc3) {
               int aa = 0;
               ++aa;
            }
            return (lastIdx, toolingSegs[lastIdx].Curve.End, toolingSegs[lastIdx].Vec1.Normalized (), 1.0);
         }

         // 6. Find the target position using existing method
         var targetPos = NotchToolPath.PositionAtParam (toolingSegs, targetLength / pathLength, tolerance);
         if (!targetPos.HasValue)
            throw new InvalidOperationException ($"Failed to find position at length {lengthFromFromPt} from starting point");
      ...
      public static (int segIndex, double param, double segParam, Point3 position)? ParamAtPositionFromPointAtLength (
         List<ToolingSegment> toolingSegs,
         Point3 fromPt,
          double lengthFromFromPt,
          double tolerance = 1e-6) {
         var res = NotchToolPath.PositionFromPointAtLength (toolingSegs, fromPt, lengthFromFromPt, tolerance);
         if (!res.HasValue) return null;
         var res2 = NotchToolPath.ParamAtPosition (toolingSegs, res.Value.position, tolerance);
         if (!res2.HasValue) return null;
         return (res2.Value.segIndex, res2.Value.param, res2.Value.segParam, res.Value.position);
      }
      #endregion
