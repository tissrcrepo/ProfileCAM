...
//          (mCurve.Clone (), mVec0, mVec1, NotchSectionType)) {
...
public class Tooling {
   ...
   public Tooling Clone () {
      ...
      // ---- Bound ------------------------------------------------------------
      if (this.mBound3.HasValue)
         cloned.mBound3 = this.mBound3;

      // ---- Segments ---------------------------------------------------------
      cloned.mSegs = [.. this.mSegs.Select (seg => new ToolingSegment (seg.Curve.Clone (), seg.Vec0, seg.Vec1, seg.NotchSectionType))];

      cloned.mCorrectedSegs = [.. this.mCorrectedSegs.Select (seg => new ToolingSegment (seg.Curve.Clone (), seg.Vec0, seg.Vec1, seg.NotchSectionType))];

      cloned.mModifiedSegs = [.. this.mModifiedSegs.Select (seg => new ToolingSegment (seg.Curve.Clone (), seg.Vec0, seg.Vec1, seg.NotchSectionType))];

      // ---- Traces -----------------------------------------------------------
      cloned.Traces.AddRange (
          this.Traces.Select (trace => (trace.Ent, trace.Trace.Clone ()))
      );

      // ---- PostRoute --------------------------------------------------------
      cloned.PostRoute = [.. this.PostRoute.Select (pv => new PointVec (pv.Pt, pv.Vec))];

      // ---- RefTooling (shallow copy to avoid recursion) ---------------------
      cloned.RefTooling = this.RefTooling;
      // If deep clone required → handle separately with graph tracking
