...
Function LicensePageCreate
  !insertmacro LogMessage "=== Creating License Page with RTF ==="
  
  ; Check if RTF license file exists
  IfFileExists "C:\FluxSDK\FChassis-License-Agreement.rtf" license_exists
    !insertmacro LogError "RTF License file not found: C:\FluxSDK\FChassis-License-Agreement.rtf"
    MessageBox MB_OK|MB_ICONSTOP "License agreement file not found. Installation cannot continue."
    Abort
  
license_exists:
  !insertmacro LogMessage "RTF License file found, creating license page"
  ...
  ; Create license text area label
  ${NSD_CreateLabel} 0 0 100% 20u "Please read the following license agreement:"
  Pop $0
  
  ; Create RichEdit control for RTF content
  ; Using RichEdit20A for better compatibility
  System::Call 'USER32::CreateWindowEx(i 0, t "RichEdit20A", t "", i ${WS_VISIBLE}|${WS_CHILD}|${WS_TABSTOP}|${WS_VSCROLL}|${ES_MULTILINE}|${ES_READONLY}, i 0, i 25, i 100%, i 130, p $hwnd, i 0, i 0, i 0) i .r1'
  Pop $LicenseRichEdit
  
  ${If} $LicenseRichEdit == 0
    !insertmacro LogError "Failed to create RichEdit control"
    ; Fallback to regular text control
    ${NSD_CreateText} 0 25u 100% 130u ""
    Pop $LicenseRichEdit
    SendMessage $LicenseRichEdit ${EM_SETREADONLY} 1 0
    ${NSD_AddStyle} $LicenseRichEdit ${WS_VSCROLL}|${WS_HSCROLL}|${ES_MULTILINE}|${ES_AUTOVSCROLL}|${ES_AUTOHSCROLL}|${ES_READONLY}
    Goto load_rtf_content
  ${EndIf}
  
  ; Set RichEdit properties
  SendMessage $LicenseRichEdit ${EM_SETEVENTMASK} 0 ${ENM_LINK}
  SendMessage $LicenseRichEdit ${EM_AUTOURLDETECT} 1 0
  
load_rtf_content:
  ; Load RTF content from file
  !insertmacro LogMessage "Loading RTF license content from: C:\FluxSDK\FChassis-License-Agreement.rtf"
  
  ; Method 1: Use StreamIn to load RTF file directly into RichEdit
  FileOpen $0 "C:\FluxSDK\FChassis-License-Agreement.rtf" r
  ${If} $0 == ""
    !insertmacro LogError "Failed to open RTF license file for reading"
    ; Fallback: Show error message
    SendMessage $LicenseRichEdit ${WM_SETTEXT} 0 "STR:Error: Could not read license agreement file."
    Goto create_buttons
  ${EndIf}
  ...
  ${If} $2 == 0
    !insertmacro LogError "Failed to allocate memory for RTF content"
    FileClose $0
    SendMessage $LicenseRichEdit ${WM_SETTEXT} 0 "STR:Error: Could not load license content."
    Goto create_buttons
  ${EndIf}
  
  ; Read file into memory
  System::Call 'kernel32::ReadFile(i $0, i $2, i $1, *i .r3, i 0)'
  FileClose $0
  
  ; Stream the RTF content into RichEdit control
  System::Call '*(i, i, i, i, i, i, i, i, i) i .r4'
  System::Call '*$4(i 0, i 0, i 0, i $2, i $1, i 0)'
  SendMessage $LicenseRichEdit ${EM_STREAMIN} ${SF_RTF} $4
  System::Free $2
  System::Free $4
  
  !insertmacro LogMessage "RTF license content loaded successfully"
...
decline_confirmed:
  !insertmacro LogMessage "User confirmed license decline, cleaning up and exiting"
  
  ; Perform cleanup if already installed
  ${If} $InstalledState != 0
    !insertmacro LogMessage "Performing uninstall due to license decline"
    ; Uninstall existing version
    ExecWait '"$INSTDIR\Uninstall.exe" /S _?=$INSTDIR'
  ${EndIf}
  
  ; Delete installer and exit
