...
public class PathUtils {
   /// <summary>
   /// Converts a given path to a valid Windows directory path.
   /// </summary>
   ...
   public static string ConvertToWindowsPath (string inputPath, bool isFile = false) {
      ...
      string windowsPath = inputPath.Replace ('/', '\\');

      // Step 2: Remove invalid characters
      char[] invalidChars = Path.GetInvalidPathChars ();
      foreach (char invalidChar in invalidChars) {
         windowsPath = windowsPath.Replace (invalidChar.ToString (), string.Empty);
      }
...
public partial class FolderPicker {
   ...
   public virtual bool? ShowDialog (IntPtr owner, bool throwOnError = false) {
      ...
      for (var i = 0; i < count; i++) {
         ...
         if (path != null && name != null) {
            mResultPaths.Add (path);
            mResultNames.Add (name);
         }
      }
      return true;
   }
   ...
   static extern int SHCreateItemFromParsingName (
    ...
    IBindCtx? pbc,  // Make the parameter nullable
