...
public class FCArc3 : FCCurve3 {
   ...
   public FCArc3 (Point3 s, Point3 i1, Point3 i2, Point3 e, Vector3 planeNormal, double tol = 1e-6) {
      ...
      var vs2i1 = (i1 - s).Normalized ();
      var vs2i2 = (i2 - s).Normalized ();
      var cross = vs2i1.Cross (vs2i2);
   ...
   public bool IsPointOnCurve (Point3 pt, double tol = 1e-6) {
      Plane pl = new (Center, Start, End);
      if (!pl.Contains (pt, tol))
         return false;
   ...
   public List<FCArc3> SplitArc (List<Point3> interPointsList,
      double deltaBetweenArcs, double tol = 1e-6) {
      ...
      for (int ii = 0; ii < interPointsList.Count; ii++) {
         if (!IsPointOnCurve (interPointsList[ii], tol))
      ...
      if (points.Count > 2) {
         ...
         for (int ii = 0; ii < points.Count - 1; ii++) {
            List<Point3> twoIntermediatePoints = GetTwoIntermediatePoints (newIncrStPt, points[ii + 1]);
