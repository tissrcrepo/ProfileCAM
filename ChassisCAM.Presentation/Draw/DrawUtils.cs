...
using ChassisCAM.Core.Tools;
using Flux.API;


namespace ChassisCAM.Presentation.Draw {
   ...
   public class ProcessSimulator (GenesysHub gHub, Dispatcher dsp) : INotifyPropertyChanged {
         ...
         while (true) {
            ...
            if (trfObject0 == null && trfObject1 == null && SimulationStatus != ESimulationStatus.NotRunning) {
               ...
               if (GenesysHub.CutScopeTraces.Count > 1 && GetCutScopeIndex () + 1 < GenesysHub.CutScopeTraces.Count) {
                  ...
                  if (ChassisCAM.Core.MCSettings.It.EnableMultipassCut)
                     ChassisCAM.Core.MCSettings.It.StepLength = mPrevStepLen;
      ...
      public void DrawToolInstance () {
         if (SimulationStatus == ESimulationStatus.Running) {
            int head = 0;
            if (ChassisCAM.Core.MCSettings.It.Heads == ChassisCAM.Core.MCSettings.EHeads.Right) head = 1;
            if (ChassisCAM.Core.MCSettings.It.Heads == ChassisCAM.Core.MCSettings.EHeads.Both) DrawToolSim (3);
